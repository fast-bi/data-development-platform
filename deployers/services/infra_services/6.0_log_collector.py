...
logger = logging.getLogger('log_collector_deployer')

class PlatformLogCollector:
    def __init__(self, chart_version, customer, metadata_collector, cloud_provider, domain_name,
                 ...
                 project_id=None, cluster_name=None, kube_config_path=None, namespace="logging", dry_run=False):
        ...
        self.chart_repo_name = "prometheus-community"
    ...
    def run(self):
        ...
        logger.info(f"Starting Platform Log Collector deployment for customer: {self.customer}")
        try:
            # Render and deploy
            self.render_values_file()
            
            # Deploy service
            logger.info("Deploying log collector service")
            self.deploy_service(
            ...
            logger.info("Platform Log Collector deployment completed successfully")
            return "Platform Log Collector deployed successfully"
    ...
    def from_cli_args(cls, args):
        """Create a PlatformLogCollector instance from CLI arguments"""
        logger.info("Creating PlatformLogCollector instance from CLI arguments")
        return cls(
...
if __name__ == "__main__":
    ...
    log_file = "log_collector_deployment.log"
    file_handler = logging.FileHandler(log_file)
    file_handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
    logger.addHandler(file_handler)
    logger.info(f"Starting Platform Log Collector deployment script, logging to {log_file}")
    
    parser = argparse.ArgumentParser(
        description="Platform Log Collector Deployment Tool",
    ...
    required_args.add_argument(
        '--chart_version',
        required=True,
        help='Chart version for Prometheus'
    )
    required_args.add_argument(
        '--cloud_provider',
        required=True,
    ...
    optional_args.add_argument(
        '--namespace',
        default='logging',
        help='Kubernetes namespace for deployment (default: logging)'
    )
    optional_args.add_argument(
        '--metadata_file',
    ...
    try:
        logger.info(f"Deploying Platform Log Collector for customer: {args.customer}")
        ...
        logger.info("Creating PlatformLogCollector instance")
        manager = PlatformLogCollector.from_cli_args(args)
        ...
        logger.info("Platform Log Collector deployment script completed")
