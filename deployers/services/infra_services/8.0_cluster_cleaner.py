...
logger = logging.getLogger('cluster_cleaner_deployer')

class Platformk8sCleaner:
    def __init__(self, chart_version, customer, metadata_collector, cloud_provider, domain_name,
                 ...
                 project_id=None, cluster_name=None, kube_config_path=None, namespace="k8s-cleanup", dry_run=False):
        ...
        self.chart_repo_name = "lwolf-charts"
    ...
    def run(self):
        ...
        logger.info(f"Starting Platform K8S Cleaner deployment for customer: {self.customer}")
        try:
            # Render and deploy
            self.render_values_file()
            
            # Deploy service
            logger.info("Deploying k8s cleaner service")
            self.deploy_service(
            ...
            logger.info("Platform K8S Cleaner deployment completed successfully")
            return "Platform K8S Cleaner deployed successfully"
...
if __name__ == "__main__":
    ...
    log_file = "cluster_cleaner_deployment.log"
    file_handler = logging.FileHandler(log_file)
    file_handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
    logger.addHandler(file_handler)
    logger.info(f"Starting Platform K8S Cleaner deployment script, logging to {log_file}")
    
    parser = argparse.ArgumentParser(
        description="Platform K8S Cleaner Deployment Tool",
    ...
    required_args.add_argument(
        '--chart_version',
        required=True,
        help='Chart version for K8S Cleaner'
    )
    required_args.add_argument(
        '--cloud_provider',
        required=True,
    ...
    optional_args.add_argument(
        '--namespace',
        default='k8s-cleanup',
        help='Kubernetes namespace for deployment (default: k8s-cleanup)'
    )
    optional_args.add_argument(
        '--metadata_file',
    ...
    try:
        logger.info(f"Deploying Platform K8S Cleaner for customer: {args.customer}")
        ...
        logger.info("Creating Platformk8sCleaner instance")
        manager = Platformk8sCleaner.from_cli_args(args)
        ...
        logger.info("Platform K8S Cleaner deployment script completed") 