...
logger = logging.getLogger('external_dns_deployer')
...
class ExternalDNS:
    def __init__(self, chart_version, customer, external_dns_domain_filters, metadata_collector, cloud_provider, domain_name,
        ...
        try:
            if self.cloud_provider == "gcp":
                ...
                self.external_dns_k8s_sa = f"external-dns-k8s-sa@{self.project_id}.iam.gserviceaccount.com"
                ...
                self.external_dns_k8s_sa = "arn:aws:iam::689791349879:role/external-dns-role" #TODO: change to be able to provide by CLI args
                self.project_id = None
                self.aws_region = None #TODO: Add in render
                self.cluster_name = cluster_name if cluster_name else f"fast-bi-{customer}-platform"
    ...
    def deploy_extra_service(self, extra_value_path, namespace):
        ...
        try:
            kubectl_command = [
                "kubectl", "apply", "-f", extra_value_path, "--namespace", namespace
            ]
    ...
    def render_values_file(self):
        ...
        try:
            # Format domain filters for YAML
            domain_filters_yaml = "[\"{}\"".format("\", \"".join(self.external_dns_domain_filters)) + "]"
    ...
    def run(self):
        ...
        logger.info(f"Starting External DNS deployment for customer: {self.customer}")
        try:
            # Deployment
            ...
            logger.info("External DNS deployment completed successfully")
            return "External DNS deployed successfully"
        except Exception as e:
    ...
    def from_cli_args(cls, args):
        """Create an ExternalDNS instance from CLI arguments"""
        logger.info("Creating ExternalDNS instance from CLI arguments")
        return cls(
            chart_version=args.chart_version,
            customer=args.customer,
            external_dns_domain_filters=args.external_dns_domain_filters,
...
if __name__ == "__main__":
    ...
    log_file = "external_dns_deployment.log"
    file_handler = logging.FileHandler(log_file)
    file_handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
    logger.addHandler(file_handler)
    logger.info(f"Starting External DNS deployment script, logging to {log_file}")
    
    parser = argparse.ArgumentParser(
        description="External DNS Deployment Tool",
    ...
    required_args.add_argument(
        '--chart_version',
        required=True,
        help='Chart version for external-dns'
    )
    required_args.add_argument(
        '--domain_name',
        required=True,
    ...
    required_args.add_argument(
        ...
        required=True,
        nargs='+',
        help='Domain filters for external DNS (space-separated list)'
    )
    required_args.add_argument(
        '--cloud_provider',
        required=True,
    ...
    optional_args.add_argument(
        ...
        help='Kubernetes namespace for deployment (default: external-dns)'
    )
    optional_args.add_argument(
        '--metadata_file',
    ...
    try:
        logger.info(f"Deploying External DNS for customer: {args.customer}")
        logger.info(f"Cloud provider: {args.cloud_provider}")
        logger.info(f"Domain filters: {args.external_dns_domain_filters}")
        ...
        logger.info("Creating ExternalDNS manager instance")
        manager = ExternalDNS.from_cli_args(args)
        ...
        logger.info("External DNS deployment script completed")