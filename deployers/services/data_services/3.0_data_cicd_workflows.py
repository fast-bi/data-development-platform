...
logger = logging.getLogger('data_cicd_workflows_deployer')

class PlatformDataCicdWorkflows:
    def __init__(self, chart_version, customer, metadata_collector, cloud_provider, domain_name,
                 ...
                 namespace="cicd-workflows", dry_run=False):
        ...
        self.deployment_name = "data-cicd-workflows"
        ...
        self.oauth_end_point_host = f"https://login.{self.customer_root_domain}/realms/{self.customer}"
        self.oauth_redirect_url = f"https://{self.ingress_host}/oauth2/callback"
    ...
    def _validate_template_paths(self):
        ...
        required_paths = [
            ...
            self.postgresql_render_template_values_path
        ]
    ...
    def render_postgresql_values_file(self):
        ...
        try:
            ...
            # Ensure output directory exists
            output_dir = os.path.dirname(self.postgresql_values_path)
            if not os.path.exists(output_dir):
                logger.info(f"Creating output directory: {output_dir}")
                os.makedirs(output_dir, exist_ok=True)
                
            self.render_template(self.postgresql_render_template_values_path, self.postgresql_values_path, context)
            logger.info(f"Successfully rendered PostgreSQL values file to {self.postgresql_values_path}")
        except Exception as e:
    ...
    def run(self):
        ...
        try:            
            ...
            if self.local_postgresql == "true":
                ...
                self.render_postgresql_values_file()
                
                # Deploy PostgreSQL first
                logger.info("Deploying PostgreSQL for Argo Workflows")
                self.deploy_service(
                ...
                self.execute_command([
                    ...
                    "-l", "app.kubernetes.io/instance=cicd-workflow-db-psql",
                    "-n", self.namespace,
                    "--timeout=300s",
                    "--kubeconfig", self.kube_config
                ])
            ...
            logger.info("Deploying Argo Workflows")
            self.deploy_service(
            ...
            logger.info("Waiting for Argo Workflows to be ready...")
            self.execute_command([
                "kubectl", "wait", "--for=condition=ready", "pod",
                "-l", "fastbi=cicd-workflows",
                "-n", self.namespace,
                "--timeout=300s",
                "--kubeconfig", self.kube_config
            ])
            ...
            logger.info("Platform Data CICD Workflows deployment completed successfully")
            return "Platform Data CICD Workflows deployed successfully"
    ...
    def from_cli_args(cls, args):
        """Create a PlatformDataCicdWorkflows instance from CLI arguments"""
        logger.info("Creating PlatformDataCicdWorkflows instance from CLI arguments")
        return cls(
...
def main():
    ...
    try:
        # Initialize the data CICD workflows
        logger.info(f"Initializing Platform Data CICD Workflows for customer {args.customer}")
        workflows = PlatformDataCicdWorkflows(
            customer=args.customer,
            chart_version=args.chart_version,
            cloud_provider=args.cloud_provider,
            domain_name=args.domain_name,
        ...
        # Deploy the data CICD workflows
        logger.info("Starting data CICD workflows deployment")
        workflows.run()
        logger.info("Data CICD workflows deployment completed successfully")
...
if __name__ == "__main__":
    ...
    log_file = "data_cicd_workflows_deployment.log"
    file_handler = logging.FileHandler(log_file)
    file_handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
    logger.addHandler(file_handler)
    logger.info(f"Starting Platform Data CICD Workflows deployment script, logging to {log_file}")
    
    parser = argparse.ArgumentParser(
        description="Platform Data CICD Workflows Deployment Tool",
    ...
    required_args.add_argument(
        '--chart_version',
        required=True,
        help='Chart version for Argo Workflows'
    )
    required_args.add_argument(
        '--cloud_provider',
        required=True,
    ...
    optional_args.add_argument(
        '--namespace',
        default='cicd-workflows',
        help='Kubernetes namespace for deployment (default: cicd-workflows)'
    )
    optional_args.add_argument(
        '--metadata_file',
    ...
    try:
        logger.info(f"Deploying Platform Data CICD Workflows for customer: {args.customer}")
        ...
        logger.info("Creating PlatformDataCicdWorkflows instance")
        manager = PlatformDataCicdWorkflows.from_cli_args(args)
        ...
        logger.info("Platform Data CICD Workflows deployment script completed")