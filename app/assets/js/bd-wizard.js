...
$(document).ready(function() {
  ...
  // Check if there's a token key in localStorage
      ...
      submitTenantDeployFinaliserRequest()
        .then(function(response) {
          ...
          setTimeout(function() {
            ...
            $('.actions > ul > li:eq(2) a').text('Exit').off('click').on('click', function(e) {
              e.preventDefault();
              // Add confirmation dialog
              if (confirm('Are you sure you want to exit? This will return you to the home page.')) {
...
function handleGenerateSecretsStep() {
  ...
  function updateNextButtonVisibility() {
      if (checkFormValidity()) {
          ...
          $('.actions > ul > li:eq(2)').show().find('a').text('Submit');
      } else {
          $('.actions > ul > li:eq(1)').hide(); // Hide Next button
      }
  }
...
function submitDeploySystemInfraServicesRequest() {
  ...
  // Combine stored whitelisted IPs with the external IP
  const storedWhitelistIPs = localStorage.getItem('whitelistIPs') || '';
  const whitelistIPArray = storedWhitelistIPs.split(',').map(ip => ip.trim()).filter(ip => ip !== '');
  
  // Add the external IP if it's available and not already in the list
  if (window.deployer_external_ip && !whitelistIPArray.includes(window.deployer_external_ip)) {
    whitelistIPArray.push(window.deployer_external_ip);
  }
  ...
  const data = {
    ...
    region: localStorage.getItem('region'),
    project_id: localStorage.getItem('projectId'),
    whitelisted_environment_ips: whitelistIPArray,
    ...
    prometheus_chart_version: storedVersions['prometheus-community/prometheus'] || '',
    grafana_chart_version: storedVersions['grafana/grafana'] || '',
    kube_cleanup_operator_chart_version: storedVersions['lwolf-charts/kube-cleanup-operator'] || ''
  };
...
function submitDeployDataServicesRequest() {
  ...
  const data = {
    ...
    airflow_chart_version: storedVersions['apache-airflow/airflow'] || '',
    airflow_app_version: storedVersions['airflow/app'] || '',
    ...
    jupyterhub_app_version: storedVersions['jupyterhub/app'] || '',
    bi_system: localStorage.getItem('dataVisualisationType'),
    superset_chart_version: storedVersions['superset/superset'] || '',
    superset_app_version: storedVersions['superset/app'] || '',
    lightdash_app_version: storedVersions['lightdash/app'] || '',
    lightdash_chart_version: storedVersions['lightdash/lightdash'] || '',
    metabase_app_version: storedVersions['metabase/app'] || '',
...
window.addEventListener('message', (event) => {
  if (event.data.token_key) {
    ...
    localStorage.setItem('tokenKey', tokenKeyValue);
  }
});
